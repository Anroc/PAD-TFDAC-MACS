...
One big advantage of TF-DAC-MACS is its constant size cipher text. It achieves this by only allowing n-of-n threshold policy. This means that a data owner is only allowed to create \textit{AND} policies to encrypt his content. However, extending this scheme to n-of-m threshold policy is not trivial. To not break any security constrains it was decided to just upload multiple versions of the same cipher text encrypted under different policies. Now, a user only needs to decipher one version of the cipher text to recover the encrypted content. 
